...
# Chooses the best next track from remaining songs
...
# Builds the best-fit song order from all scanned tracks
def build_smart_track_order(library, starting_track_id=None, preferred_mix_duration=None):
    ...
    if starting_track_id:
        ...
        # Default starting point: longest track with good phrase metadata
    ...
    remaining_tracks = [
        t for t in tracks
        if t["track_id"] != current_track["track_id"]
    ]
    ...
    while remaining_tracks:
        ...
        remaining_tracks = [
            ...
            if t["track_id"] != next_track["track_id"]
        ]
...
# Builds readable timeline showing where each track starts in the final set
def build_set_timeline(ordered_tracks, transition_results):
    ...
    for idx, track in enumerate(ordered_tracks):
        timeline.append({
            ...
            "starts_at_seconds": round(current_time, 3),
            "starts_at_timecode": seconds_to_timecode(current_time),
            "bpm": track.get("bpm"),
            "key": track.get("key"),
        ...
        if idx < len(transition_results):
            transition = transition_results[idx]
            transition_start = transition["render"]["snapped_transition_start_time"]
            track_b_entry = transition["render"]["track_b_entry_time"]
            mix_duration = transition["render"]["duration_seconds"]

            current_time += max(0, transition_start)
            current_time += mix_duration
            current_time -= track_b_entry
...
# Renders a full DJ set using the exact queue order chosen by the user
...
# Builds optimized order, renders transitions, stores setlist, and optionally assembles final set
